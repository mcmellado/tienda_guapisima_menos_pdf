                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="py-4 px-6">
                                ...
                                <?php if($cupon_factura): ?>
                                    <?php $pdo = conectar(); 
                                    $sent = $pdo->prepare("SELECT descuento FROM cupones WHERE id = :cupon");
                                    $sent->execute([':cupon' => $cupon_factura]); 
                                    $descuento_cupon = $sent->fetchColumn();  ?>
                                    <?= round(($factura->getTotal() -($descuento_cupon/100*$factura->getTotal()))*1.21, 2) ?> €
                                <?php endif ?>     
